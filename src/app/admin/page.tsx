...
import { getIssues, updateIssueStatus, IssueStatus, CATEGORIES } from "@/lib/store";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { MapPin, Search, ArrowLeft, MoreHorizontal, CheckCircle, Clock, AlertTriangle } from "lucide-react";
...
export default function AdminPage() {
    ...
    const handleStatusUpdate = (id: string, newStatus: IssueStatus) => {
        ...
        setIssues([...getIssues()]); // Force re-render with fresh data
    };
                                ...
                                {filteredIssues.length > 0 ? (
                                    filteredIssues.map((issue) => (
                                        <TableRow key={issue.id} className="cursor-pointer hover:bg-slate-50/50">
                                            <TableCell className="font-mono text-slate-500">#{issue.id}</TableCell>
                                            ...
                                            <TableCell className="text-slate-600 truncate max-w-[200px]">{issue.location}</TableCell>
